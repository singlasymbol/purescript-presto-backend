...
runKVDB brt dbName kvDBAct mockedKvDbActDictF rrItemDict = do
  ...
  case eitherConn of
    Right (Redis simpleConn) -> RightEx <$> withRunModeClassless brt rrItemDict
        (runKVDB' brt dbName simpleConn kvDBAct)
    Right (MockedKVDB mocked) -> RightEx <$> withRunModeClassless brt rrItemDict
...
runEitherKvDB
  ...
  -> InterpreterMT' rt st eff (EitherEx DBError a)
runEitherKvDB brt dbName kvDBAct mockedKvDbActDictF rrItemDict = do
  eitherConn <- getKVDBConn' brt dbName
  case eitherConn of
